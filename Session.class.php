...
class Session {
  ...
  private $maxHops = -1;         // nombre maximal d'accés à la session, -1 => pas de limite
  private $maxAge = -1;          // durée de vie maximale d'une session, -1 => pas de limite
  ...
  public function start() {
    ...
    if (isset($_SESSION['time']) && isset($_SESSION['remainHops'])) {
      // dans ce cas où la session est déjà active, on vérifie si elle doit se poursuivre
      if (($this->maxAge  !== -1 && time() - $_SESSION['time'] > $this->maxAge) ||
	  ($this->maxHops !== -1 && $_SESSION['remainHops'] <= 0))
      {
	// la session est trop vieille ou trop usée, il faut la détruire
	session_destroy();
	return false;
      } else {
	$_SESSION['remainHops']--;
	return true;
      ...
      $_SESSION['time'] = time();
      $_SESSION['vars'] = [];  // le tableau des variables de session
      return true;
    }
  }
  ...
  public function setRemainingHops() {
    return $_SESSION['remainHops'];
  }
  
  // accesseur au temps de validité restant
  public function setRemainingTime() {
